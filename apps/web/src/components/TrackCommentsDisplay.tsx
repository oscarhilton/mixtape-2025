...
export function TrackCommentsDisplay({ playlistId, comments }: TrackCommentsDisplayProps) {
  ...
  useEffect(() => {
    ...
    const commentsForCurrentTrack = comments.filter(
      (comment) => comment.track_uri === currentTrack.uri && comment.playlist_id === playlistId
    );
  ...
  return (
    ...
    >
      {activeComments.map((comment) => (
        <div key={comment.id}>
          <p><strong>User {comment.user_id} says:</strong> {comment.comment_text}</p>
