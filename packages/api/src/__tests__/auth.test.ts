...
jest.mock('passport', () => ({
  ...
  authenticate: (...args: any[]) => {
    ...
    return (req: any, res: any, next: any) => {
      ...
      if (args[1]?.successRedirect || args[1]?.failureRedirect) { // Likely a callback
        if (mockPassportAuthenticate.mock.calls.some(call => call[1]?.simulateSuccess)) {
            req.user = { id: 1, spotify_id: 'mockspotifyid', display_name: 'Mock User' };
...
describe('Auth Routes', () => {
  ...
  describe('GET /auth/spotify', () => {
    it('should attempt to authenticate with Spotify', async () => {
      ...
      expect(mockPassportAuthenticate).toHaveBeenCalledWith('spotify', expect.anything());
    });
  });
  ...
  describe('GET /auth/spotify/callback', () => {
    it('should redirect to frontend on successful authentication', async () => {
      ...
      mockPassportAuthenticate.mockImplementation((strategy, options) => {
        return (req: any, res: any, next: any) => {
          req.user = { id: 1, spotify_id: 'testuser', display_name: 'Test User' };
      ...
      const mockUser = { id: 1, spotify_id: 'user123', display_name: 'Callback User' };
      ...
      expect(mockPassportAuthenticate).toHaveBeenCalledWith('spotify', { failureRedirect: '/login-failed' });
  ...
  describe('GET /auth/me', () => {
    it('should return user info if authenticated', async () => {
      const mockUser = { id: 1, spotify_id: 'testuser', display_name: 'Test User' };
      ...
      app.use((req, res, next) => { // Temporary middleware to mock authentication
    ...
    it('should return 401 if not authenticated', async () => {
        app.use((req, res, next) => { // Temporary middleware to mock NO authentication
  ...
  describe('GET /auth/logout', () => {
    it('should logout the user and redirect or send success message', async () => {
      ...
      expect((app as any)._router.stack[ (app as any)._router.stack.length-1 ].handle.clearCookie).toHaveBeenCalledWith('connect.sid');
      (app as any)._router.stack.pop(); 
    });
  });
}); 