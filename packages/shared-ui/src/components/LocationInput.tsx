...
export function LocationInput({ onLocationChange, initialLocation }: LocationInputProps) {
  const [locationMethod, setLocationMethod] = useState<'automatic' | 'manual'>('automatic');
  const [address, setAddress] = useState(initialLocation?.address || '');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [location, setLocation] = useState<Location>(initialLocation || { latitude: null, longitude: null });
      ...
      async (position) => {
        ...
        try {
          ...
          if (data.display_name) {
            ...
            onLocationChange({ latitude, longitude, address: data.display_name });
          }
        } catch (err) {
    ...
    <div className="space-y-4">
        ...
        <form onSubmit={handleAddressSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-spotify-dark border border-spotify-gray rounded-md text-white placeholder-spotify-light-gray
      ...
      {isLoading && (
        <div className="text-center text-spotify-light-gray">
          {locationMethod === 'automatic' ? 'Getting your location...' : 'Looking up address...'}
        </div>
      )}
